# AirDropd Detailed Documentation

## Overview

AirDropd is a cross-platform application designed to bridge the gap between Windows and Apple ecosystems by implementing the AirDrop and AirPlay protocols. This allows users to seamlessly share files and screens between Windows and Apple devices.

## Architecture

AirDropd is structured into modular components for maintainability and scalability:
...
AirDropd uses mDNS (Multicast DNS) for service discovery and advertisement.  The application binds to port 5353 and listens for multicast traffic on 224.0.0.251.  Specific socket options are configured to ensure reliable multicast communication.
...
1. **Clone the repository:** `git clone https://github.com/seregonwar/AirDropd.git`
2. **Navigate to the project directory:** `cd AirDropd`
