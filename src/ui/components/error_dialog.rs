...
pub fn from_error<E: std::fmt::Display>(error: E) -> ErrorDialog {
    ...
    let (title, message, details) = if error_str.contains("10048") || error_str.contains("port") || error_str.contains("bind") {
        (
            ...
            "Another application is already using the requested port. AirDropd will continue with limited functionality.".to_string(),
            Some(error_str),
        )
        ...
        (
            ...
            "The application does not have the required permissions. Try running it as administrator.".to_string(),
            Some(error_str),
        )
    } else {
        (
