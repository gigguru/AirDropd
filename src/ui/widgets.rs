...
pub fn airdrop_radar<'a>(theme: &IcedTheme, is_scanning: bool, tick: u32) -> Element<'a, Message> {
    ...
    container(
        container(
            ...
            .center_x()
            .center_y()
            .style(ring_style(120.0, inner_fill, 0.35))
        )
        .width(Length::Fixed(180.0))
        .height(Length::Fixed(180.0))
        .center_x()
        .center_y()
        .style(ring_style(180.0, mid_fill, 0.25))
    )
    .width(Length::Fixed(240.0))
    .height(Length::Fixed(240.0))
    .center_x()
    .center_y()
    .style(ring_style(240.0, outer_fill, 0.18))
    .into()
}

/// macOS AirDrop-style device bubble
pub fn device_bubble<'a>(
    device_name: &str,
    device_icon: &str,
    is_selected: bool,
    theme: &IcedTheme,
    ...
    let bubble_bg = if is_selected {
        Color::from_rgba(0.0, 0.48, 1.0, if is_dark { 0.25 } else { 0.15 })
    } else if is_dark {
