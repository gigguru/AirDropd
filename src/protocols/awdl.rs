...
//! into the AirDropd project, providing Apple Wireless Direct Link protocol support.
...
use crate::utils::{AirDropdError, AirDropdResult};
use std::sync::Arc;
use tokio::sync::RwLock;
use tracing::{info, warn, error, debug};
use serde::{Deserialize, Serialize};

/// AWDL Manager for AirDropd integration
#[derive(Debug)]
...
/// AWDL peer information for AirDropd
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Default for AwdlManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            interface: None,
            device_name: "AirDropd-Device".to_string(),
            service_name: "_airdropd._tcp".to_string(),
            auto_start: true,
...
impl AwdlManager {
    ...
    pub async fn initialize(&mut self) -> AirDropdResult<()> {
        ...
        match builder.build().await {
            Ok(mut daemon) => {
                ...
                if let Err(e) = daemon.init().await {
                    ...
                    return Err(AirDropdError::NetworkError(format!("AWDL initialization failed: {}", e)));
                }
            ...
            Err(e) => {
                ...
                self.set_state(AwdlManagerState::Error).await;
                Err(AirDropdError::NetworkError(format!("AWDL daemon build failed: {}", e)))
            }
        }
    }

    /// Start the AWDL manager
    pub async fn start(&mut self) -> AirDropdResult<()> {
        if !self.config.enabled {
            return Ok(());
        }
        ...
        if let Some(ref mut daemon) = self.daemon {
            match daemon.start().await {
                ...
                Err(e) => {
                    ...
                    self.set_state(AwdlManagerState::Error).await;
                    Err(AirDropdError::NetworkError(format!("AWDL start failed: {}", e)))
                }
            }
        } else {
            error!("AWDL daemon not initialized");
            self.set_state(AwdlManagerState::Error).await;
            Err(AirDropdError::NetworkError("AWDL daemon not initialized".to_string()))
        }
    }

    /// Stop the AWDL manager
    pub async fn stop(&mut self) -> AirDropdResult<()> {
        ...
        if let Some(ref mut daemon) = self.daemon {
            match daemon.stop().await {
                ...
                Err(e) => {
                    ...
                    self.set_state(AwdlManagerState::Error).await;
                    Err(AirDropdError::NetworkError(format!("AWDL stop failed: {}", e)))
                }
            }
        } else {
            self.set_state(AwdlManagerState::Stopped).await;
    ...
    pub async fn broadcast_data(&self, data: &[u8]) -> AirDropdResult<()> {
    ...
    pub async fn update_config(&mut self, config: AwdlManagerConfig) -> AirDropdResult<()> {
...
mod tests {
    ...
    fn test_awdl_manager_config_default() {
        ...
        assert_eq!(config.device_name, "AirDropd-Device");
        assert_eq!(config.service_name, "_airdropd._tcp");
    }
